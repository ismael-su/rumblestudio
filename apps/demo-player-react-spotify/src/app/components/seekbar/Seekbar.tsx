...
export default class Seekbar extends React.Component<SProps, SState> {
	...
	componentDidMount() {
		this.context.percentage.asObservable().subscribe((percentage) => {
	...
	render() {
		return (
			<div className={'barContainer'}>
				<div
					className={'barHolder'}
					ref={this.barRef as React.RefObject<HTMLDivElement>}
					onClick={(event) => this.onClick(event)}
				>
					<div
						className={'bar'}
						ref={
							this.progressRef as React.RefObject<HTMLDivElement>
						}
					/>
				</div>
			</div>
	...
	private updateBar(percentage: number) {
		(this.progressRef.current as HTMLDivElement).style.width =
			100 * percentage + '%';
	}

	private onClick(event) {
		const bcr = (
			this.barRef.current as HTMLDivElement
