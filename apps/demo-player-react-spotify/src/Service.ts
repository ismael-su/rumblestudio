import { PlayerService, Song } from '@rumblestudio/player-service';
...
export class Bridge {
	...
	public isShuffled: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
		false,
	);
	public isLooping: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
		false,
	);
	...
	constructor() {
		...
		this.playlist.next([]);
		this.player.addNewOnCallback((event) => {
			this.on(event);
		});
